...
# pip install streamlit scikit-learn python-docx PyPDF2 Pillow
# Note: pdf2image and pytesseract are optional (only needed for scanned PDFs)
...
def load_models():
    try:
        ...
        # If not found in models folder, try current directory
        try:
            svc_model = pickle.load(open('clf.pkl', 'rb'))
            tfidf = pickle.load(open('tfidf.pkl', 'rb'))
            le = pickle.load(open('encoder.pkl', 'rb'))
            ...
            st.error(f"Model file not found: {e}. Please ensure clf.pkl, tfidf.pkl, and encoder.pkl are in the same directory or 'models' folder.")
            return None, None, None
